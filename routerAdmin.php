...
<body>
    ...
    if (isset($_GET['act']) && $_GET['act']) {
        ...
        switch ($_GET['act']) {
            ...
            case 'binhluan_chitiet':
                include "view/admin/binhluan_chitiet/index.php";
                break;
            case 'binhluan_sua':
                if (isset($_POST['luu'])) {
                    $commentId = $_GET['commentId'];
                    $replyContent = isset($_POST['replyContent']) ? trim($_POST['replyContent']) : '';
                    $status = $_POST['status'];

                    $updateDetail = $comment->update_commentDetail($commentId, $replyContent);
                    $updateComment = $comment->update_commentStatus($commentId, $status);
